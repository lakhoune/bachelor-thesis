...
Each service is a \emph{node} in the network. Many nodes can form a peer-to-peer seed network, which communicate through standard protocols. 
...
New nodes can join the las2peer network by using the \emph{bootstrap} flag followed by the IP adress of one of the nodes in the network. The service can be adressed by its \emph{ServiceAgent} id or by listening to an arbitrary topic. It can adress other services in the network in the same way. 
...
In order to communicate with the nodes of the las2peer network from outside, a \emph{WebConnector} can be started on a node. This connector handles REST requests to the services. A service can then be accessed over http by the following url sheme: 
\begin{lstlisting}
...
\footnotetext{\href{https://github.com/rwth-acis/mobsos-success-modeling}{Github: Success Modeling}}
...
A Docker \emph{container} is a standalone executable package that contains all depencies for the actual application. Docker containers run using the Docker \emph{engine}. Docker \emph{images} define the way a container should run, while the container is the actual runtime.
...
Docker images can be uploaded to Docker registries\footnotemark, wich are central repositories for Docker images. 
...
K8s introduces an abstractation layer to containers called \emph{pods}. Pods can contain multiple containers. Pods are created and managed automatically by K8s. K8s also introduces \emph{services}, which serve as the communication endpoint of the pod, as it maps a single IP adress to a pod. This is necessary as the IP adress of the pod can change if it is restarted.
Different application can communicate with each other via \texttt{http}. 
